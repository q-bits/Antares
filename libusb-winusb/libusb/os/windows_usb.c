...
int get_vid_pid(char *str, int *vid, int *pid)
{
	char *s2;
	...
	if (s2!=NULL)
	{
		if (2==sscanf(s2, "vid_%4x&pid_%4x",vid,pid))
			return 1;
	}
...
struct husb_device_handle global_handle;

set_pipe_timeout(struct husb_device_handle *fd, int ep, ULONG timeout)
{

	WinUsb_SetPipePolicy( fd->usbHandle,
		ep,
...
struct husb_device_handle* open_winusb_device(HANDLE hdev)
{
	...
	r = WinUsb_QueryInterfaceSettings(fd.usbHandle, 0, &iface);
	if (!r) return NULL;

	fd.read_timeout=0; fd.write_timeout=0;
	for(i=0;i<iface.bNumEndpoints;i++)
	{
		r = WinUsb_QueryPipe(fd.usbHandle, 0, (UCHAR) i, &pipeInfo);
		if (!r)
			printf("WinUsb_QueryPipe, %s. \n",windows_error_str(0));
...
int find_usb_paths(char *dev_paths,  int *pids, int max_paths,  int paths_max_length, char *driver_names, int specified_pid)
{
	...
	DWORD reg_type, install_state;

	char path[128];
	char driver_str[MAX_KEY_LENGTH];

	dev_info = pSetupDiGetClassDevsA(&GUID_DEVINTERFACE_USB_DEVICE,NULL,NULL,DIGCF_PRESENT |DIGCF_DEVICEINTERFACE);
	...
	for (i=0; i<999; i++)
	{
		...
		pSetupDiGetDeviceInterfaceDetailA(dev_info, &dev_interface_data, NULL, 0, &requiredLength, NULL);


		dev_interface_details = (SP_DEVICE_INTERFACE_DETAIL_DATA_A*) realloc(dev_interface_details,requiredLength);
		...
		if (dev_interface_details==NULL) break;
		
		if (!get_vid_pid(dev_interface_details->DevicePath,&vid,&pid)) continue;
		...
		if ( (!pSetupDiGetDeviceRegistryPropertyA(dev_info, &dev_info_data, SPDRP_INSTALL_STATE,
			&reg_type, (BYTE*)&install_state, 4, &requiredLength))
			|| (requiredLength != 4) ){
				printf("could not detect installation state of driver.\n");
		} else if (install_state != 0) {
			printf("driver for device is reporting an issue (code: %d) - skipping\n",
				install_state);
			continue;
		}


		if (pSetupDiGetDeviceRegistryPropertyA(dev_info,& dev_info_data, SPDRP_SERVICE,
	...
	if (dev_interface_details) free(dev_interface_details);
					   ...
					   uint8_t port_number, char* device_id, DWORD devinst)
{
	...
	if (parent_priv->apib->id != USB_API_HUB) {
		...
		if (NULL!=strstr(device_id,"11DB") && (NULL!=strstr(device_id,"1000") || NULL !=strstr(device_id,"1100") ) )
		{
	...
	if (priv->depth != 0 && !force_toppy) {	// Not a HCD hub
		...
		else
		{
			...
			priv->dev_descriptor.idVendor = (uint16_t)0x11db;
			if (NULL!=strstr(device_id,"1000"))
				priv->dev_descriptor.idProduct = (uint16_t)0x1000;
			else
				priv->dev_descriptor.idProduct = (uint16_t)0x1100;
			dev->num_configurations = 0;
			priv->dev_descriptor.bLength = sizeof(USB_DEVICE_DESCRIPTOR);
			priv->dev_descriptor.bDescriptorType = USB_DEVICE_DESCRIPTOR_TYPE;
			priv->dev_descriptor.bNumConfigurations = 0;
